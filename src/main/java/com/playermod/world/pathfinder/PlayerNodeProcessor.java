package com.playermod.world.pathfinder;
...
public class PlayerNodeProcessor extends NodeProcessor {
	private boolean canEnterDoors;
	private boolean canBreakDoors;
	private boolean avoidsWater;
	private boolean canSwim;
	private boolean shouldAvoidWater;
	private List<PathPoint> options = new ArrayList<PathPoint>();
	...
	public PathPoint getPathPointTo(Entity entityIn) {
		...
		if (this.canSwim && entityIn.isInWater()) {
			...
			BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor_double(entityIn.posX), i,
					MathHelper.floor_double(entityIn.posZ));

			for (Block block = this.blockaccess.getBlockState(blockpos$mutableblockpos).getBlock(); block == Blocks.flowing_water || block == Blocks.water; block = this.blockaccess
					.getBlockState(blockpos$mutableblockpos).getBlock()) {
				++i;
	...
	private PathPoint getSafePoint(Entity entityIn, int x, int y, int z, int p_176171_5_) {
		...
		if (i == 2) {
			this.options.add(new PathPoint(x, y, z));
	...
	private int getVerticalOffset(Entity entityIn, int x, int y, int z) {
		return func_176170_a(this.blockaccess, entityIn, x, y, z, this.entitySizeX, this.entitySizeY, this.entitySizeZ, this.avoidsWater, this.canBreakDoors,
				this.canEnterDoors);
	}

	public static int func_176170_a(IBlockAccess blockaccessIn, Entity entityIn, int x, int y, int z, int sizeX, int sizeY, int sizeZ, boolean avoidWater,
			boolean breakDoors, boolean enterDoors) {
		boolean flag = false;
	...
	public PathPoint[] getAllPoints() {
		...
		PathPoint[] points = this.options.toArray(new PathPoint[this.options.size()]);
		this.options.clear();
		return points;
